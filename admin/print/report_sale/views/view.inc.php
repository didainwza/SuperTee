...
for($page_index=0 ; $page_index < $page_max ; $page_index++){
    ...
    <div align="center" style="font-size:14px;color:#00F;"> <b>รายงานยอดสั่งซื้อ</b></div>
    ...
    for($i=$page_index * $lines; $i < count($data) && $i < $page_index * $lines + $lines; $i++){ 
                $net_total += intval($data[$i]['net_total']);
                $html[$page_index] .= ' 
                <tr>  
                    <td align="center" >'.($i + 1).'</td>
                    <td align="center">'.$data[$i]['member_firstname'].' &nbsp; &nbsp;&nbsp; &nbsp;'.$data[$i]['member_lastname'].'</td>
                    <td align="right">'.$data[$i]['net_total'].'</td>
                </tr> 
                ';
    }
    $html[$page_index] .= ' 
                <tr>
                    <td align="right" colspan="2" >ยอดรวมทั้งสิ้น</td>
                    <td align="right" colspan="1" >'.number_format($net_total,2, '.', ',').'</td>
                </tr>
    </tbody> 
</table>
'; 
